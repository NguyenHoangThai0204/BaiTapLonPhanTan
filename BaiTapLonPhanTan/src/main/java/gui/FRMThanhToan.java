...
//        }
        ArrayList<MatHang> lsMH = daoMH.getAllMatHangs();
        for(MatHang mh : lsMH) {
            jComboBoxTenMH.addItem(mh.getTenMatHang());
        }
        jTMaNV.setText(maTK);
    ...
    private void jButtonDanhSachHoaDonActionPerformed(java.awt.event.ActionEvent evt) throws RemoteException {//GEN-FIRST:event_jButtonDanhSachHoaDonActionPerformed
        // TODO add your handling code here:
...
//   		jTMaNV.setText(p.getMaPhong());
        ...
        DonDatPhong ddp = daoDDP.getDDPTheoMa(p.getMaPhong());
    ...
    public void timKiem() {
        try {
            if(regex.regexTimKiemMaPhong(jTextFieldTimHD)) {
                Phong p1 = daoPhong.getPhongDangHoatDongTheoMa(jTextFieldTimHD.getText().toString());
                if(p1!=null)
                    loadInfo(p1);
                else
...
//	  @return boolean
//
    public boolean kiemTraMatHangTrongBang(ChiTietDDP ctddp, MatHang mh) throws RemoteException {
        if(timRow() != -1) {
    ...
    public void themMHVaoCTDDP() throws RemoteException {
        if(jTextField1.getText() != "") {
            if(regex.regexSoLuong(jTSoLuong)) {
                DonDatPhong ddp = (DonDatPhong) daoDDP.getDDPTheoMaDDP(jTextField1.getText());
...
//	  Trước khi xóa phải chọn một mặt hàng cần xóa
    ...
    public void xoaCTDDP() throws RemoteException {
        // mã phòng
        if(!jTextField1.getText().equalsIgnoreCase("")) {
            if(timRow() != -1) {
                DonDatPhong ddp = daoDDP.getDDPTheoMa(jTextField1.getText());
